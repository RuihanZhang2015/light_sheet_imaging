...
def plot_mean_intensity(mean_intensity, camera_index, save_dir = '/om2/user/ruihanz/zeguan/image'):
    '''
...
def plot_stitched(out_path, layer_index = 0, save_dir = '/om2/user/ruihanz/zeguan/image'):
    '''
...
def combining_two_cameras(layer_path, stitch_path, desired_len, offset_time):
    '''
    Input:
        layer_path: path to the volume from each camera, a string with {} as the placeholder for camera index. It has datasets as f[f'layer{layer_index}'], with the shape of (256, 1280, t).
    ...
    # Compute mean intensity of the first few frames of each z layer
    mean_intensity_cam1 = compute_mean_intensity(layer_path.format(1))
    mean_intensity_cam2 = compute_mean_intensity(layer_path.format(2))
    ...
    print('---------------------------')
    stitching(
        cam1_path = layer_path.format(1),
        cam2_path = layer_path.format(2),
...
if __name__ == '__main__':
    ...
    # Define paths
    layer_path = '/nese/mit/group/boydenlab/ruihan/FISHDATA/VOLTAGE/20230826_gal4_3xPosi2_xCaspr_F2_5-6dpf_40us_4980us_UV/fish1/camera{}/fish1_1.h5'
    ...
    # TEST
    combining_two_cameras(layer_path, stitch_path, desired_len = 50, offset_time = 50) 
